...
function HomePageContent({ initialLang }: { initialLang?: 'zh' | 'en' }) {
  ...
  const t = {
    zh: {
      ...
      emptyText: '未找到相关工具',
      sitemapTitle: '网站地图',
      sectionWhatTitle: '实用工具能帮你做什么',
      sectionWhatBody: '实用工具是一套精选的 10 个浏览器端在线计算与实用工具,涵盖折扣计算、BMI、睡眠周期、日期倒计时、农历转换、单位换算、QR 码生成、字数统计、JSON 格式化、HEIC 转 JPG、发票生成。无需注册、无需下载、无跟踪,所有处理都在浏览器本地完成。',
      sectionWhoTitle: '适合谁使用',
      sectionWhoBody: '适合比价购物者、学生、项目经理、中英双语用户、开发者、摄影师、自由职业者和写作者。每个工具都为单一任务设计——打开页面、完成操作、关闭标签页。',
      sectionHowTitle: '工作原理',
      sectionHowBody: '每个工具都是静态 Next.js 页面,完全在浏览器中通过客户端 JavaScript 执行。无需账户、无需跟踪像素、无需服务器处理你的输入。文件（如 HEIC 图片和 JSON 文档）不会离开你的设备。页面加载后,JavaScript 被浏览器缓存,支持离线使用。',
      sectionTrustTitle: '结果可信吗',
      sectionTrustBody: '折扣、BMI、日期、单位和字数统计计算器使用标准公开公式。农历转换基于香港天文台历表（见下方权威参考）。BMI 计算器使用 WHO 1995 年制定、2004 年确认的成人 BMI 分类标准。',
      sectionAuthoritativeTitle: '权威参考',
      sectionAuthoritativeBody: '我们的工具基于以下公开标准和参考来源：',
      refWHO: '世界卫生组织 — BMI 分类标准',
      refHKO: '香港天文台 — 农历历表',
      refISO: 'ISO/IEC 18004:2015 — QR 码标准',
      ...
      faq3Q: '可以离线使用吗？',
      faq3A: '可以。页面加载后,JavaScript 被浏览器缓存,无需网络连接即可继续使用。',
      faq4Q: '支持中文吗？',
      faq4A: '支持。每个工具页面都有英文（默认）和简体中文版本。通过 ?lang=en 或 ?lang=zh 切换语言。',
      faq5Q: '计算结果准确吗？',
      faq5A: '计算遵循公开标准：WHO BMI 阈值、SI 单位定义、ISO/IEC 18004 QR 码标准、香港天文台农历历表。',
      sectionCtaTitle: '开始使用',
    },
    en: {
